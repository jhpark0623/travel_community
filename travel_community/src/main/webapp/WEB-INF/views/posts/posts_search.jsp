...
<body>
	...
	<div class="container my-5">
					...
					<c:forEach items="${postsList}" var="dto">
						<tr>
							...
							<td class="text-start position-relative p-0"><a
								href="${pageContext.request.contextPath}/posts_search_content.go?no=${dto.id}&page=${paging.page}"
			...
			<ul class="pagination justify-content-center">
				<c:if test="${paging.page > paging.block}">
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/posts_list.go/${i}?page=1">처음</a>
					</li>
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/posts_list.go/${i}?page=${paging.startBlock - 1}">이전</a>
					</li>
				</c:if>
				...
				<c:forEach begin="${paging.startBlock}" end="${paging.endBlock}"
					...
					<li class="page-item ${i == paging.page ? 'active' : ''}"><a
						class="page-link"
						href="${pageContext.request.contextPath}/posts_list.go/${i}?page=${i}">${i}</a>
					</li>
				</c:forEach>

				<c:if test="${paging.endBlock < paging.allPage}">
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/posts_list.go/${i}?page=${paging.endBlock + 1}">다음</a>
					</li>
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/posts_list.go/${i}?page=${paging.allPage}">마지막</a>
					</li>
				</c:if>
			</ul>
		</nav>
		...
		<!-- ✅ 검색 영역 -->
		<form method="post"
			action="${pageContext.request.contextPath}/posts_search.go"
